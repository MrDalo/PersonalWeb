...
const contentBoxesPositions = Array.from(contentBoxes).map((box) =>
...
function ArrowBurgerColorChange(pageNumber) {
	if (pageNumber % 2 >= 1) {
		...
		Array.from(burgerMenuIcon.getElementsByTagName("div")).forEach(
			(element) => (element.style.backgroundColor = mainBrown)
		);
	} else {
		upArrow.getElementsByTagName("path")[0].style.fill = mainGray;
		downArrow.getElementsByTagName("path")[0].style.fill = mainGray;
		Array.from(burgerMenuIcon.getElementsByTagName("div")).forEach(
			(element) => (element.style.backgroundColor = mainGray)
		);
	}
}
...
function CareerSliderRendering(direction) {
	if (direction == "up") {
		Array.from(careerItems).forEach((item) =>
		...
		careerItems[career[4] - 1].classList.add("last-hidden-job");
		...
		careerItems[career[3] - 1].classList.add("hidden-job");
		careerItems[career[4] - 1].classList.add("hidden-job");
	}

	if (career[0] > career[1]) {
		document.getElementById("list-of-jobs").style.flexDirection =
			"column-reverse";
	} else {
...
function CareerUpArrow() {
	...
	Array.from(document.getElementsByClassName("hidden-job")).forEach(
		(item) => {
			item.style.top = "0px";
	...
	document.getElementsByClassName("last-hidden-job")[0].style.animation =
		"last-hidden-job-animation-up 1s";
	document.getElementsByClassName("last-hidden-job")[0].style.top = "0px";

	document.getElementsByClassName("first-job")[0].style.animation =
		"first-job-animation-up 1s";
	document.getElementsByClassName("second-job")[0].style.animation =
		"second-job-animation-up 1s";
}
...
function CareerDownArrow() {
	...
	document.getElementsByClassName("last-hidden-job")[0].style.animation =
		"last-hidden-job-animation-down 1s";
	document.getElementsByClassName("last-hidden-job")[0].style.bottom = "0px";

	document.getElementsByClassName("first-job")[0].style.animation =
		"first-job-animation-down 1s";
	document.getElementsByClassName("second-job")[0].style.animation =
		"second-job-animation-down 1s";
}

function PortfolioSliderRendering(direction) {
	if (direction == "up") {
		Array.from(portfolioProjects).forEach((item) =>
			item.classList.remove(
				"first-project",
				"second-project",
				"hidden-project",
		...
		portfolioProjects[portfolio[0] - 1].classList.add("first-project");
		portfolioProjects[portfolio[1] - 1].classList.add("second-project");
		portfolioProjects[portfolio[2] - 1].classList.add(
			"last-hidden-project"
		);
		portfolioProjects[portfolio[3] - 1].classList.add("hidden-project");
		portfolioProjects[portfolio[4] - 1].classList.add("hidden-project");
	...
	if (portfolio[0] > portfolio[1]) {
		document.getElementById("projects").style.flexDirection =
			"column-reverse";
	} else {
...
function PortfolioUpArrow() {
	...
	Array.from(document.getElementsByClassName("hidden-project")).forEach(
		(item) => {
			item.style.top = "0px";
	...
	document.getElementsByClassName("last-hidden-project")[0].style.animation =
		"last-hidden-project-animation-up 1s";
	document.getElementsByClassName("last-hidden-project")[0].style.top = "0px";

	document.getElementsByClassName("first-project")[0].style.animation =
		"first-project-animation-up 1s";
	document.getElementsByClassName("second-project")[0].style.animation =
		"second-project-animation-up 1s";
...
function PortfolioDownArrow() {
	...
	document.getElementsByClassName("last-hidden-project")[0].style.animation =
		"last-hidden-project-animation-down 1s";
	document.getElementsByClassName("last-hidden-project")[0].style.bottom =
		"0px";

	document.getElementsByClassName("first-project")[0].style.animation =
		"first-project-animation-down 1s";
	document.getElementsByClassName("second-project")[0].style.animation =
		"second-project-animation-down 1s";
...
function MainContentBoxScrollEventFunction() {
	...
	let offset =
		window.innerWidth > 1024
			? contentBoxesPositions[2].height * 0.3
			: contentBoxes[2]
					.getElementsByClassName("left-box")[0]
					.getBoundingClientRect().height *
					0.2 +
			  contentBoxes[2]
					.getElementsByClassName("right-box")[0]
					.getBoundingClientRect().height;

	if (IsInViewport(contentBoxes[2], offset)) {
		Array.from(progressBars).forEach((bar) =>
			bar.getElementsByTagName("div")[0].classList.add("bar-visible")
		);
	} else {
		Array.from(progressBars).forEach((bar) =>
			bar.getElementsByTagName("div")[0].classList.remove("bar-visible")
		);
	}
	...
	if (!scrollAction && !smallerThan1024px) {
		if (pages.page != 6 && IsInViewport(contentBoxes[pages.page], 50)) {
			...
			pages.page != 1 &&
			IsInViewport(contentBoxes[pages.page - 2], 50)
		) {
			scrollAction = true;
 ...
 */
document
	.getElementById("main-content-box")
	.addEventListener("scroll", MainContentBoxScrollEventFunction);
...
window.onload = (event) => {
	...
	if (window.innerWidth > 1024) {
		...
		Array.from(contentBoxes).forEach(
			(item) => (item.style.scrollSnapAlign = "start")
		);
		document
			.getElementById("main-content-box")
			.addEventListener("scroll", MainContentBoxScrollEventFunction);
		...
		setInterval(() => {
			if (!scrollAction) {
				contentBoxesArray.forEach((box, i) => {
					if (IsInViewport(box, 50)) {
						pages.page = i + 1;
						ArrowBurgerColorChange(pages.page);
					}
				});
